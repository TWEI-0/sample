...
<?php
		$get_categories_args = array("parent" => 0);
		$categories = get_categories($get_categories_args);
		foreach($categories as $category) show_child_category($category, 1);
		function show_child_category($parent_category, $depth) {
			...
			foreach($posts as $post) {
				...
				if (count($post_category) > $depth) continue;
?>
				<li><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></li>
<?php
			}
			$get_categories_args = array("parent" => $parent_category->cat_ID);
			$categories = get_categories($get_categories_args);
